...
export const StrategyDashboard: React.FC<StrategyDashboardProps> = ({ 
  ...
  const toggleDay = (dayValue: number) => {
    setSelectedDays((prev: number[]) =>
      prev.includes(dayValue) ? prev.filter((d: number) => d !== dayValue) : [...prev, dayValue].sort((a: number, b: number) => a - b)
    );
  };
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {scenarios.map((strategy: Scenario) => (
                          ...
                          <div className="space-y-5">
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex gap-2">
                                 {['weekdays', 'weekends', 'all', 'clear'].map((preset: string) => (
                                   <button
                                     key={preset}
                                  ...
                                  {selectedDays.length > 0 && manualTimes.length > 0 ? (
                                    <>
                                      ...
                                      on <span className="font-bold">
                                        {selectedDays.length === 7 ? 'Every Day' : selectedDays.map((dv: number) => DAYS.find((d: any) => d.value === dv)?.label).join(', ')}
                                      </span> 
